 ...
 *      
 *      sexyControllerData.button1 = 1;   // Marks the triangle button as pressed
 *      sexyControllerData.button3 = 0;     // Marks the square button as unpressed
...
#define UNOJOY_H
	...
	{
		uint8_t button1 : 1;  // Each of these member variables
		uint8_t button2 : 1;    //  control if a button is off or on
		uint8_t button3 : 1;    // For the buttons, 
		uint8_t button4 : 1;     //  0 is off
		uint8_t button5 : 1;        //  1 is on
		uint8_t button6 : 1;        
		uint8_t button7 : 1;        // The : 1 here just tells the compiler
		uint8_t button8 : 1;        //  to only have 1 bit for each variable.
        ...
        uint8_t padding : 5;     // We end with 7 bytes of padding to make sure we get our data aligned in bytes
